...
namespace Carfaith.Aplicacion.ServiciosImpl
{
    public class OrdenEgresoServicioImpl : IOrdenEgresoServicio
    {
        ...
        public async Task UpdateOrdenEgresoAsync(OrdenEgreso ordenEgreso)
        {
            ...
            {
                existeOrdenEgreso.Fecha = ordenEgreso.Fecha;
                existeOrdenEgreso.Estado = ordenEgreso.Estado;
                existeOrdenEgreso.Destino = ordenEgreso.Destino;
            }
            else
            {
                throw new KeyNotFoundException($"Orden de egreso con ID {ordenEgreso.IdOrdenEgreso} no encontrada.");
            }

            await _ordenEgresoRepositorio.UpdateAsync(existeOrdenEgreso!);
        }
    }
}
